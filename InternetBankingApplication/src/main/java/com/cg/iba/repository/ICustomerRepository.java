...
public interface ICustomerRepository extends JpaRepository<Customer, Long>{
	...
	@Query(value="select * from Customer where customer_id in (select customer_id from customers_accounts where account_id in (select account_id from account where min_balance=?1) )",nativeQuery=true)                                                      
